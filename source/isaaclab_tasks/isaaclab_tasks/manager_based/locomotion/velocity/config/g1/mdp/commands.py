...
class ScaledVelocityCommand(UniformVelocityCommand):
    """Velocity command that mirrors another command with independent per-axis scale factors.
    ...
    each component independently:
      - ``x_scale`` multiplies the source's forward (x) velocity.
      - ``y_scale`` multiplies the source's lateral (y) velocity.
      - ``yaw_scale`` multiplies the source's yaw velocity.

    Setting ``x_scale=1.0, y_scale=-1.0`` creates a forward-mirrored conflict
    (same forward speed, opposite lateral direction) while preserving equal
    total XY speed magnitude between the two commands.
    """
    ...
    def __init__(self, cfg: ScaledVelocityCommandCfg, env: ManagerBasedEnv):
        ...
        self._x_scale = cfg.x_scale
        self._y_scale = cfg.y_scale
    ...
    def _resample_command(self, env_ids):
        ...
        self.vel_command_b[env_ids, 0] = source_cmd[env_ids, 0] * self._x_scale
        self.vel_command_b[env_ids, 1] = source_cmd[env_ids, 1] * self._y_scale
    ...
    def _update_command(self):
        ...
        source_cmd = self._env.command_manager.get_command(self._source_command_name)
        self.vel_command_b[:, 0] = source_cmd[:, 0] * self._x_scale
        self.vel_command_b[:, 1] = source_cmd[:, 1] * self._y_scale
...
class ScaledVelocityCommandCfg(UniformVelocityCommandCfg):
    ...
    x_scale: float = 1.0
    """Multiplier applied to the source command's forward (x) velocity."""

    y_scale: float = 1.0
    """Multiplier applied to the source command's lateral (y) velocity. Set to -1.0 to mirror laterally."""
